...
const records = new SqliteRecordRepository(db); const media = new SqliteMediaRepository(db); const messages = new SqliteMessageRepository(db); const sessions = new SessionManager((opts) => createAgentRuntime(opts)); const queue = new LocalMediaQueue(); const ai = { apiKey: process.env.DASHSCOPE_API_KEY ?? "", baseUrl: process.env.DASHSCOPE_BASE_URL ?? "https://ws-2gkw6cbbhgg7bqz5.cn-beijing.maas.aliyuncs.com/compatible-mode/v1" };
registerImageUnderstandingListener(queue, records, media, oss, new QwenImageUnderstanding(ai.apiKey, ai.baseUrl));
const server = serve({ fetch: createApp(records, media, queue, oss, ai, config, sessions, messages).fetch, port: config.port, hostname: config.host });
logInfo("main", "Server listening", { host: config.host, port: config.port });
const shutdown = async () => { server.close(); sessions.cleanupAll(); await db.destroy(); process.exit(0); }; process.on("SIGINT", shutdown); process.on("SIGTERM", shutdown);
