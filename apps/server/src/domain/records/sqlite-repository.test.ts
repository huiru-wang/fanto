...
test("record HTTP accepts source and requires a valid user header", async () => {
  ...
  try {
    ...
    const upload = await app.request("/api/uploads", { method: "POST", headers: { "Content-Type": "application/json", "x-user-id": "u" }, body: JSON.stringify({ fileName: "memo.m4a", mediaType: "audio", mimeType: "audio/mp4", bytes: 3 }) });
    assert.equal(upload.status, 201); assert.equal((await upload.json() as any).result.uploadUrl, "https://upload.example");
